# Week 14 — RDKit Descriptors and ETL
...
**Depends on:** weeks 00–13

**Topic scope:** Parsing SMILES with RDKit, computing a small fixed descriptor set, and storing derived features with provenance.
...
```bash
make test-week WEEK=week-14-rdkit-descriptors-and-etl
```
...
- [ ] `make test-week WEEK=week-14-rdkit-descriptors-and-etl` passes.
...
Save to `docs/portfolio/week-14/`:
...
```bash
git add -A
git commit -m "<type>(week-14): <imperative summary>"
```
