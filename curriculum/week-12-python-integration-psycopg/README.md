# Week 12 — Python Integration with psycopg
...
**Depends on:** weeks 00–11

**Topic scope:** Connections, cursors, parameterised queries, SQL-injection prevention and transaction control from Python.
...
```bash
make test-week WEEK=week-12-python-integration-psycopg
```
...
- [ ] `make test-week WEEK=week-12-python-integration-psycopg` passes.
...
Save to `docs/portfolio/week-12/`:
...
```bash
git add -A
git commit -m "<type>(week-12): <imperative summary>"
```
