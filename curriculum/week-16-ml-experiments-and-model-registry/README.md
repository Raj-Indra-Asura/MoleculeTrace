# Week 16 — ML Experiments and the Model Registry
...
**Phase:** 4. ML registry · **Required effort:** 5 hours ·
**Depends on:** weeks 00–15

**Topic scope:** Recording experiments, model versions, metrics, predictions and validation results with scikit-learn — kept deliberately simple.
...
```bash
make test-week WEEK=week-16-ml-experiments-and-model-registry
```
...
- [ ] `make test-week WEEK=week-16-ml-experiments-and-model-registry` passes.
...
Save to `docs/portfolio/week-16/`:
...
```bash
git add -A
git commit -m "<type>(week-16): <imperative summary>"
```
