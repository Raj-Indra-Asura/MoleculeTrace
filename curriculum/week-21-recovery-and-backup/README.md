# Week 21 — Recovery, Backup and Restore
...
**Depends on:** weeks 00–20

**Topic scope:** Failure classes, write-ahead logging, checkpoints, redo and undo, and a dump-destroy-restore drill on the real database.
...
**Indexed in labs/:** [`labs/recovery.md`](../../labs/recovery.md)
...
```bash
make test-week WEEK=week-21-recovery-and-backup
```
...
- [ ] `make test-week WEEK=week-21-recovery-and-backup` passes.
...
Save to `docs/portfolio/week-21/`:
...
```bash
git add -A
git commit -m "<type>(week-21): <imperative summary>"
```
