...
**Depends on:** weeks 00–03

**Topic scope:** Functional dependencies, candidate keys, 1NF to BCNF, lossless join and dependency preservation applied to the activity table.
...
**Indexed in labs/:** [`labs/normalization.md`](../../labs/normalization.md)
...
```bash
make test-week WEEK=week-04-normalization
```
...
- [ ] `make test-week WEEK=week-04-normalization` passes.
...
Save to `docs/portfolio/week-04/`:
...
```bash
git add -A
git commit -m "<type>(week-04): <imperative summary>"
```
