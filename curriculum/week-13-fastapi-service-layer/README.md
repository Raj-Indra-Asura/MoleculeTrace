# Week 13 — A FastAPI Service Layer
...
**Depends on:** weeks 00–12

**Topic scope:** Pydantic schemas, request validation, read and write endpoints over molecules, assays and activities, and API tests.
...
```bash
make test-week WEEK=week-13-fastapi-service-layer
```
...
- [ ] `make test-week WEEK=week-13-fastapi-service-layer` passes.
...
Save to `docs/portfolio/week-13/`:
...
```bash
git add -A
git commit -m "<type>(week-13): <imperative summary>"
```
