...
<!--
  ...
  Copy this whole folder to curriculum/week-XX-<slug>/ and replace every
  <angle-bracket placeholder> and TODO. Do not delete sections: every week must
  contain all of them.
-->

**Phase:** <phase from ROADMAP.md> · **Required effort:** 5 hours ·
**Depends on:** weeks 00–<XX-1>
...
```bash
make test-week WEEK=week-XX-<slug>
```
...
- [ ] `make test-week WEEK=week-XX-<slug>` passes.
...
Save to `docs/portfolio/week-XX/`:
...
```bash
git add -A
git commit -m "<type>(week-XX): <imperative summary>"
```
