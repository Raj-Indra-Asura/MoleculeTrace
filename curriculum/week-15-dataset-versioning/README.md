...
**Depends on:** weeks 00–14

**Topic scope:** Immutable dataset versions, membership tables, deterministic splits and reproducibility guarantees.
...
```bash
make test-week WEEK=week-15-dataset-versioning
```
...
- [ ] `make test-week WEEK=week-15-dataset-versioning` passes.
...
Save to `docs/portfolio/week-15/`:
...
```bash
git add -A
git commit -m "<type>(week-15): <imperative summary>"
```
