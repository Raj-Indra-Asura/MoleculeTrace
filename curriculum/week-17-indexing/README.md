# Week 17 — Indexing and Access Paths
...
**Depends on:** weeks 00–16

**Topic scope:** Selectivity, statistics, single-column, composite, partial and covering indexes, measured with EXPLAIN (ANALYZE, BUFFERS).
...
**Indexed in labs/:** [`labs/indexing.md`](../../labs/indexing.md)
...
```bash
make test-week WEEK=week-17-indexing
```
...
- [ ] `make test-week WEEK=week-17-indexing` passes.
...
Save to `docs/portfolio/week-17/`:
...
```bash
git add -A
git commit -m "<type>(week-17): <imperative summary>"
```
