# Week 10 — Views, Functions and Server-Side Logic
...
**Depends on:** weeks 00–09

**Topic scope:** Views and materialised views, updatable views, stored functions and triggers that maintain derived molecular attributes.
...
```bash
make test-week WEEK=week-10-views-and-server-side-logic
```
...
- [ ] `make test-week WEEK=week-10-views-and-server-side-logic` passes.
...
Save to `docs/portfolio/week-10/`:
...
```bash
git add -A
git commit -m "<type>(week-10): <imperative summary>"
```
