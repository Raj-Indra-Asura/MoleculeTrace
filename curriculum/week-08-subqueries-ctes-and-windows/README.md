# Week 08 — Subqueries, CTEs and Window Functions
...
**Phase:** 2. Query depth · **Required effort:** 5 hours ·
**Depends on:** weeks 00–07

**Topic scope:** Nested and correlated subqueries, EXISTS, common table expressions, recursion and window functions for per-target rankings.
...
```bash
make test-week WEEK=week-08-subqueries-ctes-and-windows
```
...
- [ ] `make test-week WEEK=week-08-subqueries-ctes-and-windows` passes.
...
Save to `docs/portfolio/week-08/`:
...
```bash
git add -A
git commit -m "<type>(week-08): <imperative summary>"
```
